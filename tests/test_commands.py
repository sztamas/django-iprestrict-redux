...
class AddIPToIPGroupTest(TestCase):
    ...
    def assertTooFewArguments(self, exception):
        self.assertTrue(
            "too few arguments" in str(exception)
            or "the following arguments are required" in str(exception)
        )
    ...
    def test_should_add_single_ip_to_ip_group(self):
        ...
        ip_range = models.IPRange.objects.get(
            ip_group__name=self.BLACKLIST, first_ip=self.IP1
        )
    ...
    def test_should_NOT_add_ip_if_ip_already_in_group(self):
        ...
        call_command(self.CMD, self.BLACKLIST, self.IP1)
        ip_count_same = models.IPRange.objects.filter(
            ip_group__name=self.BLACKLIST
        ).count()
        call_command(self.CMD, self.BLACKLIST, self.IP1, self.IP2)
        ip_count_adds_other = models.IPRange.objects.filter(
            ip_group__name=self.BLACKLIST
        ).count()
        ...
        self.assertEqual(
            ip_count_adds_other, 2, "should add the other IP but not the same IP"
        )
