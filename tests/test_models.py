...
class RuleTest(TestCase):
    ...
    def test_rank_is_automatically_assigned_on_creation(self):
        ...
        self.assertEqual(rule1.rank, 1)
        self.assertEqual(rule2.rank, 2)
        self.assertEqual(rule10.rank, 10)
        self.assertEqual(rule11.rank, 11)

    def test_rank_is_not_changed_on_update(self):
        rule1 = models.Rule.objects.create(url_pattern="1", action="A")
        _ = models.Rule.objects.create(url_pattern="2", action="A")
        rule1.action = "D"
        rule1.save()
        self.assertEqual(rule1.rank, 1)
...
class RangeBasedIPGroupTest(TestCase):
    ...
    def test_matches_with_ranges(self):
        ...
        models.IPRange.objects.create(
            ip_group=ipgroup, first_ip="192.168.1.1", last_ip="192.168.1.10"
        )
        models.IPRange.objects.create(
            ip_group=ipgroup, first_ip="192.168.1.100", last_ip="192.168.1.110"
        )
    ...
    def test_matches_with_subnets(self):
        ipgroup = models.RangeBasedIPGroup.objects.create(name="Local IPs")
        models.IPRange.objects.create(
            ip_group=ipgroup, first_ip="192.168.1.0", cidr_prefix_length=30
        )
